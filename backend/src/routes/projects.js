...
router.post('/:id/materials', authenticate, (req, res) => {
  ...
  db.prepare(
    `INSERT INTO project_materials (id, date, project_id, product_id, quantity, created_by)
     VALUES (?, ?, ?, ?, ?, ?)`
  ).run(id, date || new Date().toISOString().slice(0, 10), req.params.id, product_id, quantity, req.user.id);
  logAction({ user: req.user, action: 'project.material_log', entityType: 'project_material', entityId: id, details: { project_id: req.params.id, product_id, quantity } });
